...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                /*
                 * Audit fix (Critical remediation) — scoped to storage/app
                 * rather than the whole codebase: the application source
                 * is now tracked in git (a separate recovery mechanism —
                 * see the project's own .gitignore), so this backup exists
                 * for the one thing git doesn't cover — the database and
                 * runtime file uploads (public Media Library assets,
                 * private Application documents). Backing up the whole
                 * `base_path()` too would be redundant and needlessly slow
                 * this down on shared hosting.
                 */
                'include' => [
        ...
        'destination' => [
            ...
            /*
             * The disk names on which the backups will be stored. Audit
             * fix (Critical remediation) — a dedicated 'backups' disk
             * (config/filesystems.php), not 'local' (that's the private
             * Application documents disk this backup also reads from —
             * mixing the two would have the backup try to back up its
             * own growing archive directory).
             */
            'disks' => [
                'backups',
            ],
    ...
    'notifications' => [
        ...
        'mail' => [
            // Audit fix — was the package's own placeholder address;
            // BACKUP_NOTIFICATION_EMAIL is new (see .env.example),
            // defaulting to the existing admissions/contact address so
            // a failed/succeeded backup notification always reaches a
            // real inbox even before an operator sets this explicitly.
    ...
    'monitor_backups' => [
        [
            'name' => env('APP_NAME', 'laravel-backup'),
            // Audit fix — matches destination.disks above ('backups'),
            // not the package default 'local' (this app's 'local' disk
            // means something else entirely — see config/filesystems.php).
            'disks' => ['backups'],
            'health_checks' => [
