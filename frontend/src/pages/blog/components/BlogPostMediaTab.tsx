...
interface BlogPostMediaTabProps {
  post: BlogPost;
  canEdit: boolean;
 ...
 * BlogPostController::attachMedia/detachMedia). Mirrors
 * FacultyMediaTab's gallery pattern.
 */
export function BlogPostMediaTab({ post, canEdit, onSave, onRefresh }: BlogPostMediaTabProps) {
  const { showToast } = useToast();
  const [isPickerOpen, setIsPickerOpen] = useState(false);

  async function handleAttach(item: MediaItem) {
    try {
      await api.post(ENDPOINTS.blog.media(post.id), { media_ids: [item.id] });
      await onRefresh();
  ...
  async function handleRemove(mediaId: number) {
    await api.delete(ENDPOINTS.blog.detachMedia(post.id, mediaId));
    await onRefresh();
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <section className="flex flex-col gap-4">
        ...
        {post.gallery.length === 0 && <p className="text-body-sm text-neutral-500">No gallery images yet.</p>}
      </section>
